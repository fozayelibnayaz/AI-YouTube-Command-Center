...
export default function DashboardPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      setStatus("❌ Failed to load data: " + String(e));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function analyzeVideo(id: string) {
    ...
    try {
      ...
      if (json.success) {
        ...
        setStatus("❌ Analysis failed: " + (json.error || "Unknown"));
      }
    } catch (e) {
      setStatus("❌ Analysis error: " + String(e));
    } finally {
  ...
  async function previewEvents() {
    ...
    try {
      ...
      try { json = JSON.parse(text); } catch {
        setStatus("❌ Invalid response: " + text.substring(0, 200));
        return;
      }
      if (json.success) {
        setEvents(json.events || []);
        setShowEvents(true);
        setStatus("📊 " + json.eventsDetected + " events detected (" + json.totalVideos + " videos checked)");
      } else {
        setStatus("❌ Error: " + (json.error || "Unknown") + (json.stack ? "\n" + json.stack : ""));
      }
    } catch (e) {
  ...
  async function sendSmartAlerts() {
    setEventsLoading(true);
    setStatus("📤 Sending alerts to Telegram...");
    try {
      ...
      if (json.success) {
        setEvents(json.events || []);
        setShowEvents(true);
        const successes = (json.sentResults || []).filter((r: any) => r.success).length;
        setStatus("✅ " + successes + "/" + json.sent + " alerts sent to Telegram!");
      } else {
  ...
  async function sendTelegramTest() {
    ...
    try {
      ...
      const json = await res.json();
      setStatus(json.success ? "✅ Telegram connected!" : "❌ Telegram error: " + (json.error || "Not configured"));
    } catch (e) {
      setStatus("❌ " + String(e));
    } finally {
      setSyncing(false);
  ...
  return (
    <div className="p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-1">
              {data?.channel?.title || "Loading"} &mdash; {formatNumber(data?.channel?.subscribers || 0)} subscribers &middot; {data?.videos?.length || 0} videos
              {lastSync && <span className="text-gray-600 text-xs ml-2">Synced: {lastSync}</span>}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={previewEvents} disabled={eventsLoading} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-blue-500/30 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 text-sm disabled:opacity-50">
              <Activity size={14} className={eventsLoading ? "animate-pulse" : ""} /> {eventsLoading ? "Detecting..." : "Preview Events"}
            </button>
            <button onClick={sendSmartAlerts} disabled={eventsLoading} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-sm disabled:opacity-50">
              <Send size={14} className={eventsLoading ? "animate-pulse" : ""} /> {eventsLoading ? "Sending..." : "Send Smart Alerts"}
            </button>
        ...
        {status && (
          <div className={`p-3 rounded-lg text-sm whitespace-pre-wrap ${status.includes("❌") ? "bg-red-500/10 border border-red-500/30 text-red-400" : status.includes("✅") ? "bg-green-500/10 border border-green-500/30 text-green-400" : "bg-blue-500/10 border border-blue-500/30 text-blue-400"}`}>
            {status}
          </div>
        )}
              ...
              {events.map((e: any, i: number) => (
                <div key={i} className={`flex items-start gap-3 p-3 rounded-lg border ${
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-gray-300">{e.type.replace(/_/g, " ").toUpperCase()}</p>
                    <p className="text-sm text-white mt-0.5">{e.message}</p>
                    {e.data && <div className="flex flex-wrap gap-2 mt-2">
                      {Object.entries(e.data).map(([k, v]) => (
                        <span key={k} className="text-xs bg-black/30 px-2 py-0.5 rounded text-gray-400">{k}: <span className="text-white">{String(v)}</span></span>
                      ))}
                    </div>}
                  </div>
                </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          <StatCard title="Total Engagement" value={data?.stats?.totalEngagement || 0} icon={<Heart size={18} />} color="purple" format="number" subtitle="Likes + Comments" />
        </div>
        ...
        {topVideo && worstVideo && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-xl border border-green-500/20 bg-green-500/5">
              <div className="p-4">
                <div className="flex items-center gap-2 mb-3"><Trophy size={16} className="text-yellow-400" /><span className="text-sm font-medium text-green-400">BEST Performing Video</span></div>
                <p className="text-white font-medium text-sm mb-2 line-clamp-2">{topVideo.title}</p>
                <div className="flex gap-3 text-xs text-gray-400 flex-wrap">
                  <span>Views: {formatNumber(topVideo.views)}</span><span>CTR: {topVideo.ctr}%</span><span>Retention: {topVideo.avg_view_percentage}%</span>
                ...
                <button onClick={() => analyzeVideo(topVideo.youtube_id)} disabled={analyzing[topVideo.youtube_id]} className="mt-3 text-xs text-green-400 hover:text-green-300 flex items-center gap-1 disabled:opacity-50">
                  <Brain size={12} /> {analyzing[topVideo.youtube_id] ? "Analyzing..." : analyses[topVideo.youtube_id] ? "Re-analyze" : "Why did this work?"}
                </button>
              </div>
              {analyses[topVideo.youtube_id] && (
                <div className="border-t border-green-500/20">
                  <button onClick={() => setTopExpanded(!topExpanded)} className="flex items-center gap-2 text-sm text-green-400 w-full text-left px-4 py-3">
                    <CheckCircle size={14} /><span className="font-medium">AI Analysis {topExpanded ? "▲" : "▼"}</span>
                  </button>
            ...
            <div className="rounded-xl border border-red-500/20 bg-red-500/5">
              <div className="p-4">
                <div className="flex items-center gap-2 mb-3"><AlertTriangle size={16} className="text-red-400" /><span className="text-sm font-medium text-red-400">WORST Performing Video</span></div>
                <p className="text-white font-medium text-sm mb-2 line-clamp-2">{worstVideo.title}</p>
                <div className="flex gap-3 text-xs text-gray-400 flex-wrap">
                  <span>Views: {formatNumber(worstVideo.views)}</span><span>CTR: {worstVideo.ctr}%</span><span>Retention: {worstVideo.avg_view_percentage}%</span>
                ...
                <button onClick={() => analyzeVideo(worstVideo.youtube_id)} disabled={analyzing[worstVideo.youtube_id]} className="mt-3 text-xs text-red-400 hover:text-red-300 flex items-center gap-1 disabled:opacity-50">
                  <Brain size={12} /> {analyzing[worstVideo.youtube_id] ? "Analyzing..." : analyses[worstVideo.youtube_id] ? "Re-analyze" : "Why did this fail?"}
                </button>
              </div>
              {analyses[worstVideo.youtube_id] && (
                <div className="border-t border-red-500/20">
                  <button onClick={() => setWorstExpanded(!worstExpanded)} className="flex items-center gap-2 text-sm text-red-400 w-full text-left px-4 py-3">
                    <AlertCircle size={14} /><span className="font-medium">AI Analysis {worstExpanded ? "▲" : "▼"}</span>
                  </button>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-white flex items-center gap-2"><Zap size={20} className="text-red-400" />All Videos Ranked by AI Score</h2>
            <span className="text-xs px-2 py-1 rounded-full bg-blue-500/20 border border-blue-500/30 text-blue-400">{data?.videos?.length || 0} videos</span>
          </div>
          {(!data?.videos || data.videos.length === 0) ? (
            <div className="text-center py-20 text-gray-500"><BarChart3 size={48} className="mx-auto mb-4 opacity-30" /><p>No videos found</p></div>
          ) : (
  ...
  return (
    <div className="space-y-3">
      ...
      {analysis.ai && <div className="space-y-2">
        ...
        {analysis.ai.improved_title && <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3"><p className="text-xs font-medium text-green-400 mb-1">BETTER TITLE</p><p className="text-sm text-white font-medium">{analysis.ai.improved_title}</p></div>}
        {analysis.ai.next_video_advice && <B c="pink" l="NEXT VIDEO" t={analysis.ai.next_video_advice} />}
        <p className="text-xs text-gray-600 text-center">Powered by {analysis.ai.source === "ai-powered" ? "AI" : "Built-in expert system"}</p>
      </div>}
...
function B({ c, l, t }: { c: string; l: string; t: string }) {
  ...
  return <div className={"border rounded-lg p-3 " + cls.split(" ").slice(0, 2).join(" ")}><p className={"text-xs font-medium mb-1 " + cls.split(" ")[2]}>{l}</p><p className="text-xs text-gray-300 leading-relaxed">{t}</p></div>;
}
